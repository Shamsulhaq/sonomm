from django.contrib.auth.models import User
from django.db import models


class Area(models.Model):
    name = models.CharField(max_length=100, default=None)

    def __str__(self):
        return self.name


class Customer(models.Model):
    Gender = (('M', 'Male'), ('F', 'Female'), ('O', 'Others'))

    name = models.OneToOneField(User, on_delete=models.CASCADE, unique=True)
    phone = models.CharField(max_length=15)
    area = models.OneToOneField(Area, on_delete=models.CASCADE, default=None)
    address = models.TextField()
    gender = models.CharField(choices=Gender, default=None, max_length=10, null=True, blank=True)
    image = models.ImageField(upload_to='customer', null=True)

    def __str__(self):
        return self.name.get_full_name()




